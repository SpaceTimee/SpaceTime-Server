export const onRequest = async (context: EventContext<unknown, string, unknown>) => {
  ...
  try {
    ...
    match = data.find(([domains]) => Array.isArray(domains) && domains.some(pattern => glob(pattern, domain)));
  ...
  return new Response(JSON.stringify(match), { headers: { 'Content-Type': 'application/json; charset=utf-8' } });
};
